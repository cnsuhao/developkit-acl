...
static void ls_file(acl::scan_dir& scan, const char* path, bool recursive)
{
	...
	while ((filename = scan.next_file()) != NULL)
	{
...
static void ls_dir(acl::scan_dir& scan, const char* path, bool recursive)
{
	...
	while ((dirname = scan.next_dir()) != NULL)
	{
...
static void usage(const char* procname)
{
	logger("usage: %s -h [help] -d path -t type[dir|file|all] "
		"-r [if recursive, default: false]", procname);
}
...
int main(int argc, char* argv[])
{
	...
	while ((ch = getopt(argc, argv, "hd:t:r")) > 0)
	{
		switch (ch)
		{
		case 'h':
	...
	if (mode == "dir")
		ls_dir(scan, path, recursive);
	else if (mode == "file")
		ls_file(scan, path, recursive);
	else
		ls_all(scan, path, recursive);
