...
namespace acl
{
	...
	static HWND InitInstance(const char *class_name, HINSTANCE hInstance)
	{
		...
		if (hWnd == NULL)
			logger_fatal("create window error: %s",
				acl_last_serror());
		return (hWnd);
	}
	...
	void ipc_service::create_window(void)
	{
	...
	void ipc_service::close_window(void)
	{
		...
		DestroyWindow(hWnd_);
		if (__class_name && GetClassInfoEx(hInstance_,
			__class_name, &wcx))
		{
			logger("unregister class: %s", __class_name);
	...
	ipc_service::ipc_service(int nthread, bool win32_gui /* = false */,
		bool ipc_keep /* true */)
	{
#ifdef WIN32
		if (win32_gui)
