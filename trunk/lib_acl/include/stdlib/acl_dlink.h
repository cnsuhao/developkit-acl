 ...
 * @param plink {const ACL_DLINK*} 二分块数据链对象指针
 * @param n {acl_off_t} 查询值
 * @return {ACL_DITEM*} 数据块元素
 */
ACL_API ACL_DITEM *acl_dlink_lookup(const ACL_DLINK *plink, acl_off_t n);
 ...
 * @return {ACL_DITEM*} 数据块元素
 */
ACL_API ACL_DITEM *acl_dlink_lookup2(const ACL_DLINK *plink, acl_off_t n, int *pidx);

/**
 * 从二分块数据链中查询某个范围的数据块元素地址并记录其下标位置
 * @param plink {const ACL_DLINK*} 二分块数据链对象指针
 * @param begin {acl_off_t} 查询范围的起始位置值
 * @param end {acl_off_t} 查询范围的结束位置值
 * @param pidx {int*} 存储查询数据元素结果在二分数据链中的下标位置
 * @return {ACL_DITEM*} 数据块元素
 */
ACL_API ACL_DITEM *acl_dlink_lookup_range(const ACL_DLINK *plink, acl_off_t begin,
        acl_off_t end, int *pidx);

/**
 * 从二分块数据链中查询第一个大于某个给定值的数据块元素并记录下标位置
 * @param plink {const ACL_DLINK*} 二分块数据链对象指针
 * @param off {acl_off_t} 给定比较值
 * @param pidx {int*} 存储查询数据元素结果在二分数据链中的下标位置
 * @return {ACL_DITEM*} 数据块元素
 */
ACL_API ACL_DITEM *acl_dlink_lookup_larger(const ACL_DLINK *plink, acl_off_t off, int *pidx);
 ...
 * @return {ACL_DITEM*} 数据块元素
 */
ACL_API ACL_DITEM *acl_dlink_lookup_lower(const ACL_DLINK *plink, acl_off_t off, int *pidx);

/**
 * 向二分块数据链中添加起始、结束数据块
 * @param plink {ACL_DLINK*} 二分块数据链对象指针
 * @param begin {acl_off_t} 给定起始位置值
 * @param end {acl_off_t} 给定结束位置值
 * @return {ACL_DITEM*} 新创建的数据块元素
 */
ACL_API ACL_DITEM *acl_dlink_insert(ACL_DLINK *plink, acl_off_t begin, acl_off_t end);

/**
 * 从二分块数据链中删除包含某个给定值的数据块元素
 * @param plink {ACL_DLINK*} 二分块数据链对象指针
 * @param n {acl_off_t} 给定位置值
 * @return {int} 0：表示OK，-1: 表示输入参数非法
 */
ACL_API int acl_dlink_delete(ACL_DLINK *plink, acl_off_t n);
 ...
 */
ACL_API ACL_DITEM *acl_dlink_modify(ACL_DLINK *plink, acl_off_t begin, acl_off_t end);

/**
 * 从二分数据链中删除某个数值范围的数据块集合, 删除后有可能会在内部增加新的数据块元素
 * @param plink {ACL_DLINK*} 二分块数据链对象指针
 * @param begin {acl_off_t} 需要删除范围的起始位置
 * @param end {acl_off_t} 需要删除范围的结束位置
 * @return {int} 0：表示OK，-1: 表示输入参数非法
 */
ACL_API int acl_dlink_delete_range(ACL_DLINK *plink, acl_off_t begin, acl_off_t end);
