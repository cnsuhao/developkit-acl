...
static void disable_listen(void)
{
	 ...
	 * ×¼±¸ºÃÒ²»áÒòÆä´ÓÊÂ¼þ¼¯ºÏÖÐ±»É¾³ý¶ø²»»á±»´¥·¢£¬ÕâÑùÔÚÏÂ´ÎÊÂ¼þÑ­»·Ê± select()
	 * Ëùµ÷ÓÃµÄÊÂ¼þ¼¯ºÏÖÐ¾Í²»´æÔÚ¸Ã¼àÌýÁ÷ÁË¡£
	 */
	...
	ACL_VSTREAM *stream, void *context)
{
	...
	while (i++ < acl_var_ioctl_max_accept) {
		...
		if (fd < 0) {
	        ...
	        /* Èç¹ûÎª TCP Ì×½Ó¿Ú£¬ÔòÉèÖÃ nodelay Ñ¡ÏîÒÔ±ÜÃâ·¢ËÍÑÓ³ÙÏÖÏó */
...
void acl_ioctl_server_main(int argc, char **argv, ACL_IOCTL_SERVER_FN service, ...)
{
	...
	/* ÉèÖÃ×Ó½ø³ÌÔËÐÐ»·¾³£¬ÔÊÐí²úÉú core ÎÄ¼þ */
