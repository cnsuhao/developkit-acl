...
int acl_vstream_nonb_readn(ACL_VSTREAM *fp, char *buf, int size)
{
	...
	/* ÏÈ±£Áô¶ÁÐ´³¬Ê±Ê±¼äÖµ£¬²¢½«¸ÃÁ÷µÄ³¬Ê±ÖµÖÃÎª0£¬ÒÔÃâ
	 * Æô¶¯¶Á³¬Ê±¹ý³Ì(select)¡£
	 */
...
static void *vstream_memmove(ACL_VSTREAM *fp, size_t n)
{
#if 1
	...
	/* ÎªÁË·ÀÖ¹ÄÚ´æÊý¾Ý¸²¸ÇÎÊÌâ, ²ÉÓÃÊý¾Ý´ÓÎ²²¿¿½±´·½Ê½ */
...
int acl_vstream_unread(ACL_VSTREAM *fp, const void *ptr, size_t length)
{
	...
	/* Èç¹û¶Á»º³åÖÐÇ°²¿·Ö¿Õ¼ä²»×ã, ÔòÐèÒªµ÷ÕûÊý¾ÝÎ»ÖÃ»òÀ©³ä¶Á»º³åÇø¿Õ¼ä */
	...
	if (k < 0) {
		...
		/* Èç¹û¶Á»º³åÇøºó²¿·Ö¿Õ¼ä¹»ÓÃ, ÔòÖ»ÐèºóÒÆ»º³åÇøÖÐµÄÊý¾Ý */

		if (fp->read_buf_len - fp->read_cnt > (acl_off_t) length) {
			if (fp->read_cnt > 0)
				vstream_memmove(fp, n);
		...
		/* ËµÃ÷Õû¸ö»º³åÇøµÄ¿Õ¼ä¶¼²»¹»ÓÃ, ËùÒÔÐèÒªÀ©³ä»º³åÇø¿Õ¼ä */
			...
			*ptr = ch;
			if (ch == *ptr_needle_end) {
				...
				while(1) {
					...
					/* Ô­×Ö·û´®ÓÃÍê¶øÆ¥Åä´®»¹Ã»ÓÐ±È½ÏÍê(²»Æ¥Åä) */
...
static void __bfgets_crlf_peek(ACL_VSTREAM *fp, ACL_VSTRING *strbuf, int *ready)
{
	if (fp->read_cnt <= 0)   /* XXX: sanity check */
		return;
	while (fp->read_cnt > 0) {
		ACL_VSTRING_ADDCH(strbuf, *(fp->read_ptr));
		fp->read_cnt--;
		fp->offset++;
		if (*(fp->read_ptr) == '\n') {
			*ready = 1;
...
int acl_vstream_gets_peek(ACL_VSTREAM *fp, ACL_VSTRING *strbuf, int *ready)
{
	const char *myname = "acl_vstream_gets_peek";
	int   n;

	if (fp == NULL || strbuf == NULL || ready == NULL)
	...
	*ready = 0;
	...
	if (fp->read_cnt > 0) {
		__bfgets_crlf_peek(fp, strbuf, ready);
		if (*ready)
			return ACL_VSTRING_LEN(strbuf) - n;
	}

	/* XXX: µ÷ÓÃÕßÍ¨¹ý¼ì²é *ready ÖµÀ´ÅÐ¶ÏÊÇ·ñ¶Á¹»Êý¾Ý, ÏµÍ³IO¶Á²Ù×÷³ö´í
...
static void __bfgets_no_crlf_peek(ACL_VSTREAM *fp,
...
int acl_vstream_gets_nonl_peek(ACL_VSTREAM *fp,
	ACL_VSTRING *strbuf, int *ready)
{
	...
	*ready = 0;
	...
	if (fp->read_cnt > 0) {
		__bfgets_no_crlf_peek(fp, strbuf, ready);
		if (*ready)
			return ACL_VSTRING_LEN(strbuf) - n;
	}
...
static int __bfread_cnt_peek(ACL_VSTREAM *fp, ACL_VSTRING *strbuf,
	int cnt, int *ready)
{
	int   n;
	...
	acl_vstring_memcat(strbuf, (char*) fp->read_ptr, n);
	fp->read_cnt -= n;
	fp->offset += n;
	fp->read_ptr += n;
	cnt -= n;
...
int acl_vstream_readn_peek(ACL_VSTREAM *fp, ACL_VSTRING *strbuf,
	int cnt, int *ready)
{
	const char *myname = "acl_vstream_readn_peek";
	int   cnt_saved = cnt;

	if (fp == NULL || strbuf == NULL || cnt <= 0 || ready == NULL)
	...
	*ready = 0;
	...
	if (fp->read_cnt > 0) {
		cnt -= __bfread_cnt_peek(fp, strbuf, cnt, ready);
		if (*ready)
...
static void __bfread_peek(ACL_VSTREAM *fp, ACL_VSTRING *strbuf)
{
	/* XXX: sanity check */
	if (fp->read_cnt <= 0)
		return;
	acl_vstring_memcat(strbuf, (char*) fp->read_ptr,
		(size_t) fp->read_cnt);
...
int acl_vstream_read_peek(ACL_VSTREAM *fp, ACL_VSTRING *strbuf)
{
	const char *myname = "acl_vstream_read_peek";
	int   n;

	if (fp == NULL || strbuf == NULL)
		acl_msg_fatal("%s, %s(%d): invalid input",
			myname, __FILE__, __LINE__);
...
TAG_AGAIN:
	...
	if (fp->type == ACL_VSTREAM_TYPE_FILE) {
		...
		if (n > 0) {
			...
			/* ·ÀÖ¹»º³åÇøÄÚµÄÊý¾ÝÓëÊµ¼Ê²»Ò»ÖÂ, ½ö¶ÔÎÄ¼þIOÓÐÐ§ */
			fp->read_cnt = 0;
		}
	} else
...
static int __vstream_writev(ACL_VSTREAM *fp,
...
int acl_vstream_vfprintf(ACL_VSTREAM *fp, const char *fmt, va_list ap)
{
	...
	n = acl_vstream_writen(fp, acl_vstring_str(strbuf), n);
...
int acl_vstream_buffed_vfprintf(ACL_VSTREAM *fp, const char *fmt, va_list ap)
{
	...
	n = acl_vstream_buffed_writen(fp, acl_vstring_str(strbuf), n);
	...
	size_t buflen, int rw_timeo, int fdtype)
{
	...
	/* XXX: Ö»ÓÐ·Ç¼àÌýÁ÷²ÅÐèÒªÓÐ¶Á»º³åÇø */
	/* XXX: Ä¿Ç° UDP ·þÎñ¶Ë¿ÚºÅÔÚ MASTER ¿ò¼ÜÖÐÔÝÊ±µ±¼àÌýÌ×½Ó¿ÚÓÃ£¬ËùÒÔ
	...
	fp->close_handle_lnk = acl_array_create(5);
	return fp;
}
...
acl_off_t acl_vstream_fseek2(ACL_VSTREAM *fp, acl_off_t offset, int whence)
{
	...
	if (whence == SEEK_CUR) {
		if (fp->read_cnt >= offset) {
			...
			n = -fp->read_cnt;  /* ¼ÆËã³öÕæÊµµÄÎÄ¼þÎ»ÖÃ */
			fp->read_cnt = 0;
		} else if (fp->read_cnt >= 0) {
			offset -= fp->read_cnt;
			n = offset;  /* ¼ÆËã³öÕæÊµµÄÎÄ¼þÎ»ÖÃ */
			fp->read_cnt = 0;
		} else { /* fp->read_cnt < 0 ? */
...
acl_off_t acl_vstream_fseek(ACL_VSTREAM *fp, acl_off_t offset, int whence)
{
	...
	if (whence == SEEK_CUR) {
		...
		/* Èç¹û¶ÁÊý¾ÝÖ¸Õë¾­¹ýÒÆ¶¯£¬¿ÉÒÔ½«Æä»ØÒÆ£¬ÒòÎª»º³åÇøÄÚÊý¾Ý
		 * ²¢Î´ÆÆ»µ£¬¿ÉÒÔ¸´ÓÃ
		 */
		if (fp->read_ptr > fp->read_buf) {
		...
		/* ÅÐ¶ÏÇëÇóµÄÆ«ÒÆÎ»ÖÃÊÇ·ñÔÚ¶Á»º´æÇø¼äÄÚ */
...
int acl_file_ftruncate(ACL_VSTREAM *fp, acl_off_t length)
{
	...
	* osfinfo.c
	* _open_osfhandle: ½«WIN32 APIµÄÎÄ¼þ¾ä±ú×ª»»Îª±ê×¼CµÄÎÄ¼þ¾ä±ú
	* _get_osfhandle: ¸ù¾Ý±ê×¼CÎÄ¼þ¾ä±ú²éÑ¯WIN32 APIÎÄ¼þ¾ä±ú
	* _free_osfhnd: ÊÍ·ÅÓÉ _open_osfhandle ´ò¿ªµÄ±ê×¼CÎÄ¼þ¾ä±úµÄ×ÊÔ´£¬
	*               µ«²¢²»Êµ¼Ê¹Ø±Õ¸ÃWIN32 API¾ä±ú£¬ËùÒÔ»¹µÃÒª¶ÔÆäÕæÊµ
	*               WIN32 APIÎÄ¼þ¾ä±ú½øÐÐ¹Ø±Õ
	* close.c
...
void acl_vstream_free(ACL_VSTREAM *fp)
{
	...
	if (fp->close_handle_lnk != NULL) {
		...
		/* ÒòÎªÌí¼ÓÊ±ÊÇÕýÐòµÄ, ËùÒÔÔÚÉ¾³ýÊ±ÊÇµ¹ÐòµÄ,
		...
		for (i = n - 1; i >= 0; i++) {
			...
			if (close_handle->close_fn == NULL)
				continue;
			/* Ö»Ëù½«´Ëµ÷ÓÃ·ÅÔÚ close_fn Ç°Ãæ£¬ÊÇÎªÁË·ÀÖ¹ÓÐÈËÎóÔÚ
...
int acl_vstream_close(ACL_VSTREAM *fp)
{
	...
	/* ±ØÐëÔÚµ÷ÓÃ¸÷¸ö¹Ø±Õ»Øµ÷º¯ÊýÖ®Ç°½«Á¬½Ó¹Ø±Õ£¬·ñÔò»áÓ°Ïì iocp µÄÊÂ¼þÒýÇæ
	 * µÄÕý³£¹¤×÷¡£ÔÚÊ¹ÓÃ iocp ×÷ÎªÊÂ¼þÒýÇæÊ±£¬µ±Á÷¹Ø±ÕÊ±»áµ÷ÓÃ events_iocp.c
	 * ÖÐµÄ stream_on_close£¬¸Ãº¯Êý»áÊÍ·Åµô fdp->event_read/fdp->event_write
	 * Á½¸ö¶ÔÏó£¬µ«µ±Ì×½Ó¿ÚÎ´¹Ø±ÕÊ±£¬ÕâÁ½¸ö¶ÔÏóÓÐ¿ÉÄÜ»á±» iocp Ê¹ÓÃ±£¬Ö»ÓÐ
	 * µ±Ì×½Ó¿Ú¹Ø±ÕÊ±£¬iocp ²Å²»»áÊ¹ÓÃÕâÁ½¸ö¶ÔÏóÖÐµÄ IOCP_EVENT->overlapped µÈ
	 * ³ÉÔ±. ---2011.5.18, zsx
	 */
	/*
	 * 2011.5.18 µÄ¸Ä¶¯ËäÈ»½â¾öÁËÊÂ¼þÒýÇæÎª iocp µÄÎÊÌâ£¬µ«Í¬Ê±Ôì³ÉÁË win32
	 * ´°¿ÚÏûÏ¢ÒýÇæµÄÎÊÌâ£¬ËäÈ» win32 ÏûÏ¢ÒýÇæµÄ·½Ê½ÔÚ¹Ø±ÕÌ×½Ó¿ÚÖ®Ç°»á»Øµ÷
	 * stream_on_close£¬¶ø¸Ã»Øµ÷ÒªÇóÌ×½Ó¿Ú±ØÐëÊÇ´ò¿ªµÄ£¬¼ÈÈ»¶þÕß³öÏÖÁË³åÍ»£¬
	 * Ôò iocp µÄÎÊÌâ»¹ÊÇÓÉ iocp ÒýÇæ±¾ÉíÈ¥½â¾ö°É£¬¼´ÔÚ iocp ÒýÇæµÄ
	 * stream_on_close ÖÐ£¬ÔÚÊÍ·Å fdp->event_read/fdp->event_write Ö®Ç°¹Ø±Õ
	 * Ì×½Ó¿Ú¼´¿É£¬ÔÚ acl_vstream_close ×îºóÐèÒª¹Ø±ÕÌ×½Ó¿ÚÊ±Ö»Òª¸ù¾Ý¾ä±úÊÇ·ñ
	 * ÓÐÐ§À´ÅÐ¶ÏÊÇ·ñµ÷ÓÃ¹Ø±Õ¹ý³Ì. ---2011.5.19, zsx
	 */
	/*
	if (fp->read_buf != NULL)
		acl_myfree(fp->read_buf);
	...
	*/
	...
	if (ACL_VSTREAM_SOCK(fp) != ACL_SOCKET_INVALID && fp->close_fn)
		ret = fp->close_fn(ACL_VSTREAM_SOCK(fp));
	else if (ACL_VSTREAM_FILE(fp) != ACL_FILE_INVALID
		&& fp->fclose_fn)
	{
		ret = fp->fclose_fn(ACL_VSTREAM_FILE(fp));
	}
